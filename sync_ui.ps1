...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        # Replace Preloader
        $content = [regex]::Replace($content, "(?s)<!-- Preloader Start -->.*?<!--<< Mouse Cursor Start >>-->", "$preloader_block`r`n`r`n    <!--<< Mouse Cursor Start >>-->")
        
        # Replace Offcanvas
        $content = [regex]::Replace($content, "(?s)<!-- Offcanvas Area Start -->.*?<div class=`"offcanvas__overlay`"></div>(?:\s*<div class=`"offcanvas__overlay`"></div>)*", "$offcanvas_block`n    <div class=`"offcanvas__overlay`"></div>")
        
        # Replace Header and Search bar
        # Fix encoding for the close button
        $search_block_fixed = $search_block.Replace("Ã—", "×")
        $content = [regex]::Replace($content, "(?s)<!-- Header Section Start -->.*?<!-- tp header search  -->", "$header_block`n`n    $search_block_fixed")
        
        # fallback for header if comment not found
        if ($content -notmatch "Header Section Start") {
            $content = [regex]::Replace($content, "(?s)<header.*?</header>", $header_block)
        }

        # Replace Meta/Title
        $content = [regex]::Replace($content, "(?s)<meta\s+name=`"author`".*?<!--<< Bootstrap min.css >>-->", "$meta_and_title`r`n    <!--<< Bootstrap min.css >>-->")
