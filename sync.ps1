...
foreach ($f in $files) {
    if (Test-Path $f) {
        $c = Get-Content $f -Raw
        # Replace Preloader
        $c = [regex]::Replace($c, "(?s)<!-- Preloader Start -->.*?<!--<< Mouse Cursor Start >>-->", "$preloader`n`n    <!--<< Mouse Cursor Start >>-->")
        # Replace Offcanvas (with fix for duplicates)
        $c = [regex]::Replace($c, "(?s)<!-- Offcanvas Area Start -->.*?<div class=`"offcanvas__overlay`"></div>(?:\s*<div class=`"offcanvas__overlay`"></div>)*", "$offcanvas")
        # Replace Header and Search bar (match precisely until next section)
        $c = [regex]::Replace($c, "(?s)<!-- Header Section Start -->.*?(?=<!-- (?:team|<< Breadcrumb|Upcoming-Movie|about-section|service|portfolio|contact|login|pricing|error) Section Start -->|<!-- tp header search  -->\z)", "$header`n`n    ")
        
        # If there are duplicate search bars, remove them
        $c = [regex]::Replace($c, "(?s)<div class=`"tp-header-search-bar.*?</div>\s*</div>\s*</div>\s*<div class=`"tp-header-search-bar.*?</div>\s*</div>\s*</div>", $header_search_part_only)
        # Replace Meta
        $c = [regex]::Replace($c, "(?s)<meta\s+name=`"author`".*?<!--<< Bootstrap min.css >>-->", "$meta`n    <!--<< Bootstrap min.css >>-->")
