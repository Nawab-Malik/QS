...
$itemStmt = $db->prepare("SELECT * FROM portfolio_items WHERE id = ?");
    ...
    if (empty($errors)) {
        $update = $db->prepare(
            "UPDATE portfolio_items
             SET category_id = ?, title = ?, subtitle = ?, image_url = ?, link_url = ?, sort_order = ?
             WHERE id = ?"
        );
        $linkValue = $linkUrl !== "" ? $linkUrl : null;
        $update->bind_param("issssii", $categoryId, $title, $subtitle, $imageUrl, $linkValue, $sortOrder, $id);
        $update->execute();
        $update->close();
        $db->close();
        header("Location: index.php");
...
<div class="admin-card">
    <h3 class="mb-4">Edit Portfolio Item</h3>
    <?php if (!empty($errors)): ?>
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <button class="theme-btn" type="submit">Update Item</button>
    </form>
</div>
